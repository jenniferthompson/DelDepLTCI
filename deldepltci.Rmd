...
## Variables involved in model
rbans_vars <- c("hx.depression", "rbans.global.score", "age.enroll", "e4.type",
                "edu", "iqcode.score.e", "charlson.score", "stroke.risk",
                "mean.modsofa.icu", "del.s.imp", "coma.s.imp", "ints.sat.l90.s",
                "icudays.sevseptic.s", "bdi.totscore")

bdi_vars <- c("hx.depression", "bdi.totscore", "age.enroll", "sex.pp",
              "charlson.score", "stroke.risk", "iqcode.score.e",
              "mean.modsofa.icu")
...
## large... but it's not)
df_lt_base <- brain.fu %>%
  dplyr::select(id, fu.period, any_outcomes,
                one_of(rbans_vars), one_of(bdi_vars),
  ...
  left_join(
    dplyr::select(
      brain.oneobs,
      id, one_of(rbans_vars), one_of(bdi_vars), one_of(dcfd_vars),
      bmi, race.wb, smoke, alcohol
    ),
    by = "id"
  ) %>%
...
We know that RBANS global cognition scores are normally distributed, so we will use multivariable regression models for both 3- and 12-month followup to determine the association between a pre-admission reported history of depression and global cognition. Both models will adjust for the following potential confounders, measured at ICU admission, summarized over the ICU course, or (in the case of BDI) measured concurrent with the outcome:
...
Except for IQCODE (which has too little variability to allow splines), all continuous covariates will be allowed to have a nonlinear relationship with our outcome using restricted cubic splines.
...
We have `r nrow(df_lt_3)` patients included in our long-term models at 3 months, and `r nrow(df_lt_12)` patients included at 12 months. Below, we show the proportion of missingness for each variable included in the RBANS models by time point.

```{r rbans_missing}
## -- How much missingness is in RBANS data? (Will we want to impute?) ---------
## Create a single df with missingness for both time points
misstable_rbans <- df_lt_base %>%
  filter(fu.period %in% paste(c(3, 12), "Month") & any_outcomes) %>%
  mutate(fu.period = fct_relevel(as.character(fu.period), "3 Month")) %>%
  group_by(fu.period) %>%
  dplyr::select(one_of(rbans_vars), "sex.pp") %>%
  miss_var_summary()
...
missplot_rbans <-
  ...
  labs(y = "Proportion Missing", x = "") +
  theme_minimal()

```

```{r rbans_missing_plot}
ggplotly(missplot_rbans) %>%
    layout(title = "Proportion of Missingness in RBANS Outcome Data",
           yaxis = list(title = ""),
           showlegend = FALSE)

```

Many covariates have very little missingness, but we do have quite a few missing both our outcome of RBANS global scores and important covariates, including our main exposure of pre-admission depression. These patients are likely different from patients with complete data; it is especially likely that there are differences between patients who had complete RBANS and BDI data and those who did not. Therefore, we'll use multiple imputation to reduce the possibility of bias in our results, including all patients with at least partial long-term outcomes data at a given time point.
...
## Will use restricted cubic splines for nonlinear terms; replace most
## continuous variables' names with rcs(var, 3)
rbans_mod_vars <- str_replace(rbans_vars, rcs_replace_regex, "rcs\\(\\1, 3\\)")
...
Patients with a pre-admission history of depression, as ascertained by surrogate questionnaire, have on average a difference (95% confidence interval) of `r rbans_diffci_3` points on the RBANS global battery three months after discharge; this indicates that, after adjusting for potential confounders including concurrent depression score, there is no meaningful difference in global cognition between patients with and without a pre-admission history of depression.
...
Patients with a pre-admission history of depression, as ascertained by surrogate questionnaire, have on average a difference (95% confidence interval) of `r rbans_diffci_12` points on the RBANS global battery twelve months after discharge; this indicates that, after adjusting for potential confounders including concurrent depression score, there is no meaningful difference in global cognition between patients with and without a pre-admission history of depression.
...
As noted in the RBANS section, in all long-term models, we'll include patients who had any assessment data available at a given time point (even if the outcome of interest is missing). Please refer to that section for details on missingness in patients included in these models.
...
## continuous variables' names with rcs(var, 3)
bdi_mod_vars <-
  str_replace(setdiff(bdi_vars, "bdi.totscore"), rcs_replace_regex, "rcs\\(\\1, 3\\)")
...
Patients with a pre-admission history of depression, as ascertained by surrogate questionnaire, have on average a difference (95% confidence interval) of `r bdi_orci_3` points on the RBANS global battery three months after discharge; this indicates that, after adjusting for potential confounders including concurrent depression score, there is no meaningful difference in global cognition between patients with and without a pre-admission history of depression.
...
Patients with a pre-admission history of depression, as ascertained by surrogate questionnaire, have on average a difference (95% confidence interval) of `r bdi_orci_12` points on the RBANS global battery twelve months after discharge; this indicates that, after adjusting for potential confounders including concurrent depression score, there is no meaningful difference in global cognition between patients with and without a pre-admission history of depression.
